import { CheckCheck, CircleDot, Users } from 'lucide-react'
...
export default function ChatListItem({ chat, active, onSelect }) {
    return (
        <button type="button" className={`chat-item ${active ? 'active' : ''} ${chat.unreadCount > 0 ? 'has-unread' : ''}`} onClick={() => onSelect(chat.id)}>
            <UserAvatar user={chat.type === 'group' ? chat : chat.participant} alt="" />
            ...
            {chat.type === 'group' ? <Users size={14} className="online-dot" /> : chat.participant.online && <CircleDot size={10} className="online-dot" />}
        </button>
    )
}
