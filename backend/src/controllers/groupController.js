...
export async function setGroupLock(req, res) { const group = await memberGroup(req.params.groupId, req.user._id); requireAdmin(group, req.user._id); const input = lockInput.safeParse(req.body); if (!input.success) throw httpError(400, 'locked must be a boolean'); group.locked = input.data.locked; await group.save(); const view = groupView(await populated(Group.findById(group._id))); const io = req.app.get('io'); io?.to(`group:${group._id}`).emit('group_updated', { group: view }); group.members.forEach(member => io?.to(`user:${member.id}`).emit('group_updated', { group: view })); res.json({ group: view }) }
export async function deleteGroup(req, res) { const group = await memberGroup(req.params.groupId, req.user._id); requireAdmin(group, req.user._id); if (group.avatarPublicId) await deleteAvatar(group.avatarPublicId).catch(() => {}); await Message.deleteMany({ group: group._id }); await group.deleteOne(); group.members.forEach(member => req.app.get('io')?.to(`user:${member}`).emit('group_deleted', { groupId: group._id.toString() })); res.status(204).end() }
