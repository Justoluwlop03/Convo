...
import { uploadGroupAvatar, deleteAvatar, uploadMessageImage, deleteMessageImage, deleteVoiceNote } from '../config/cloudinary.js'
...
function requireCreator(group, userId) { if (group.creator._id.toString() !== userId.toString()) throw httpError(403, 'Only the group creator can manage this group') }
...
export async function createGroupImageMessage(req, res) {
  const group = await memberGroup(req.params.groupId, req.user._id)
  if (group.locked && !includes(group.admins, req.user._id)) throw httpError(403, 'This group is locked. Only admins can send messages.')
  ...
  await Promise.all(updatedGroup.members
    ...
    .map(async member => {
      const recipientGroup = groupView(updatedGroup, await unreadCount(updatedGroup._id, member._id))
      io?.to(`user:${member._id}`).emit('group_message_received', { message: view, group: recipientGroup })
      sendMessagePush(member._id, { conversationId: updatedGroup._id.toString(), title: updatedGroup.name, text: caption || 'Sent a photo', messageId: view.id }).catch(() => {})
    }))
...
export async function createGroupVoiceMessage(req, res) {
  ...
  await Promise.all(updatedGroup.members
    ...
    .map(async member => {
      const recipientGroup = groupView(updatedGroup, await unreadCount(updatedGroup._id, member._id))
      io?.to(`user:${member._id}`).emit('group_message_received', { message: view, group: recipientGroup })
      sendMessagePush(member._id, { conversationId: updatedGroup._id.toString(), title: updatedGroup.name, text: 'Voice message', messageId: view.id }).catch(() => {})
    }))
